...
/** 是否应在当前环境提示「复制链接到微信」（非微信且无法短信登录时才提示） */
export function shouldPromptOpenInWeChat(): boolean {
  if (typeof navigator === 'undefined')
    return false
  return !isWeChatBrowser() && !isSmsLoginAvailable()
}
...
export function promptOpenInWeChat(options?: { force?: boolean }): void {
  if (isWeChatBrowser())
    return
  if (!options?.force && !shouldPromptOpenInWeChat())
    return
